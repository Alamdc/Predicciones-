...
def render_xgb_page():
    ...
    with st.expander("Hiperparámetros XGBoost", expanded=False):
    ...
    if uploaded and st.button("Entrenar y generar pronóstico"):
        with st.spinner("Entrenando modelos y generando pronóstico…"):
            try:
                ...
                st.success("¡Listo! Abajo puedes revisar métricas y descargar el CSV de pronóstico.")
                ...
                buf = io.StringIO()
                result["forecast"].to_csv(buf, index=False)
                st.download_button(
